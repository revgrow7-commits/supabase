#!/usr/bin/env python3
"""
Specific test for image compression with truly large images
"""
...
def test_large_image_compression():
    """Test image compression with a truly large image"""
    print("=" * 60)
    print("LARGE IMAGE COMPRESSION TEST")
    print("=" * 60)
    ...
    # Test with regular checkin
    print(f"\nTesting regular checkin with {original_size_kb:.1f}KB image...")
    ...
    response = session.post(f"{BASE_URL}/checkins", data=form_data, headers=headers)
    
    if response.status_code != 200:
        print(f"❌ Checkin failed: {response.status_code} - {response.text}")
        return False
    ...
    # Test checkout with another large image
    print(f"\nTesting checkout with another large image...")
    ...
    form_data = {
        ...
        "gps_accuracy": GPS_CHECKIN["accuracy"],
        "notes": "Large image compression test checkout"
    }
    
    response = session.put(f"{BASE_URL}/checkins/{checkin_id}/checkout", data=form_data, headers=headers)
    ...
    print("\n✅ Large image compression test completed successfully!")
    return True

if __name__ == "__main__":
    success = test_large_image_compression()
